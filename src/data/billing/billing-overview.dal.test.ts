...
import { category, listing } from "@/db/schema";
...
test("keepListing(): публикува избраната, скрива останалите published сестри на собственика", async () => {
  // setup correction (семантично неутрален): sibling-hide е standard-семантика; vendor беше premium
  // (от mine() теста по-горе), а plan-aware keepListing вече скрива сестри само за standard.
  await createTestSubscription(vendorId, { plan: "standard", status: "active" });
  const cityId = await getTestCityId();
...
test("keepListing(): premium — публикува избраната, published обява в ДРУГА категория остава published", async () => {
  const u = await createTestUser();
  const uu: SessionUser = { id: u.id, email: u.email, name: "Премиум Кийп", isAdmin: false };
  await createTestSubscription(u.id, { plan: "premium", status: "active" });
  const cityId = await getTestCityId();
  const [catB] = await testDb.select({ id: category.id }).from(category).where(ne(category.id, categoryId)).limit(1);
  ...
  const [rowB] = await testDb.select().from(listing).where(eq(listing.id, b.id));
  expect(rowA?.status).toBe("published");
  expect(rowA?.hiddenBySystem).toBe(false);
  expect(rowB?.status).toBe("published"); // чужда категория недокосната
  await cleanupTestUser(u.id);
});

test("keepListing(): standard swap — hidden става published, старата published се скрива (hiddenBySystem=true)", async () => {
  const u = await createTestUser();
  const uu: SessionUser = { id: u.id, email: u.email, name: "Стд Своп", isAdmin: false };
  ...
  const [rowNew] = await testDb.select().from(listing).where(eq(listing.id, hiddenId));
  expect(rowNew?.status).toBe("published");
  expect(rowNew?.hiddenBySystem).toBe(false);
  expect(rowOld?.status).toBe("hidden");
  expect(rowOld?.hiddenBySystem).toBe(true);
  await cleanupTestUser(u.id);
});
